"""Local configuration and credential storage for the TJU TUI.
...
def load_config() -> dict[str, Any]:
    """Load and return the config dict.  Returns an empty dict if the file
    does not exist yet."""
    path = _config_path()
    if not path.exists():
        return {}
...
def get_username(cfg: dict[str, Any] | None = None) -> str | None:
    """Return the stored username from *cfg* (or load it if not given)."""
    if cfg is None:
        cfg = load_config()
