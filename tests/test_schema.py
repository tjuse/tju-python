...
def test_single_schedule_schema():
    raw_dict = json.loads(
        Path(__file__)
        .parent.joinpath("resources/response/parsed_schedule_ug_std_single.json")
        .read_text()
    )
    schema = ScheduleCourse.Schema()
    schedule_course = schema.load(raw_dict)
...
def test_nested_schedule_schema():
    raw_list = json.loads(
        Path(__file__)
